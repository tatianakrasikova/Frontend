...
const addTaskBtn = document.querySelector(".actions .btn");
...
taskList.appendChild(listItem);
  ...
  // Добавляем новую задачу при нажатии на кнопку "Создать"
  addTaskBtn.addEventListener("click", () => {
    const taskText = taskInput.value.trim();
    if (taskText) {
      createTask(taskText);  // Создаём задачу
...
//Когда задача выполнена (клик по задаче), её статус изменяется с false на true и обратно.
//После каждого клика на задачу происходит перерисовка списка задач с учётом их текущего статуса. Выполненные задачи получают класс list-item_done, что меняет их стиль на зачёркнутый текст.
//Пример работы:
//При создании новой задачи она добавляется в список.
//При клике на задачу она меняет статус (выполнено/не выполнено), и её стиль (перечёркивание) обновляется.
//Кнопки "Все", "Выполненные" и "Невыполненные" работают корректно, фильтруя задачи по их статусу.
//Теперь выполненные задачи будут зачёркиваться при изменении их статуса.